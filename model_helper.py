...
#  emb = []
#  t0 = time.time()
#  for vo in trainable_tokens:
#    emb.append(tf.Variable([0.0] * emb_size))
#
#  for vo in vocab[num_trainable_tokens:]:
#    if vo in emb_dict:
#      emb.append(tf.constant(emb_dict[vo]))
#    else:
#      emb.append(tf.Variable([0.0] * emb_size))
#  
#  print('embedding matrix constructed in %f\n' % (time.time()-t0))
#
#  return tf.convert_to_tensor(emb)
  ...
  t = 0
  for vo in vocab[num_trainable_tokens:]:
    if vo not in emb_dict:
      t +=  1
      trainable_tokens.append(vo)
    else:
      emb_const.append(emb_dict[vo])
      const_tokens.append(vo)
  
#  with codecs.getreader('utf8')(tf.gfile.GFile(vocab_file+'.new', 'w')) as f:
#    for tra in trainable_tokens:
#      f.write(tra + '\n')
#    for con in const_tokens:
#      f.write(con + '\n')
#  print('reordering vocab with trainable tokens first. \n')

  t += num_trainable_tokens
  num_trainable_tokens = len(trainable_tokens)
  assert t == num_trainable_tokens
...
#      emb_dict[token] = [0.0] * emb_size
  ...
  emb_mat_const = tf.constant(np.array(emb_const, dtype=dtype.as_numpy_dtype()))
